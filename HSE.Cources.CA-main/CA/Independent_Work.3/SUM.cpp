...
void sum() {
	...
	int task;
	printf("\nInsert type of sum (0: without saturation; 1: with saturation; 2 for exit) > ");
	scanf("%d", &task);
	switch (task) {
	case 0: {
		__asm {
		...
		SUM:
			...
			paddq mm0, qword ptr[edi + 8 * ecx]		//Добавить к переменной MMX MM0 квадро слово по индексу ecx из массива В
	...
	case 1: {
		...
		SUM_S:
			movq mm0, qword ptr[esi + 8 * ecx]		//Положить в переменную MMX ММ0 квадро слово по индексу ecx из массива А
			paddsw  mm0, qword ptr[edi + 8 * ecx]	//Добавить к переменной MMX MM0 квадро слово по индексу ecx из массива В с насыщением
			movq qword ptr[edi + 8 * ecx], mm0		//Положить переменную ММ0 в массив В по индексу ecx
			inc ecx									//Перейти к след. элементу
			cmp ecx, 10								//Проверка на выход
			jne SUM_S
